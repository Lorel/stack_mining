...
public class StackTraceParser {
	...
	private final List<StackTraceMatch> traces = new ArrayList<StackTraceMatch>();
	...
	private boolean findExceptionBeforeFrame() {
		...
		while (frameMatcher.find(frameIndex)) {
			...
			Matcher subFrameMatcher = framePattern.matcher(text.substring(frameStart + whiteSpaces + 1,
					frameMatcher.end()));
	...
	private boolean findFrame() {
		...
		if (matcher.find(currentIndex) && matcher.start() == currentIndex && hasAcceptableWhiteSpaceCount(matcher)) {
			final String method = (matcher.group(1).replaceAll("/", ".") + "." + matcher.group(2))
					.replaceAll("\\s", "");
